...
def make_dataset(p_train, p_val, p_test, cuadrinorm = False,**kwargs):
    ...
    # (3) Load the Data
    if seed:
        signal = pd.read_csv('manu/signal_manu.csv', header=None)#.sample(frac=1,random_state=seed)
        background = pd.read_csv('manu/background_manu.csv', header=None)#.sample(frac=1, random_state=seed)
    else:
        signal = pd.read_csv('manu/signal_manu.csv', header=None).sample(frac=1)
        background = pd.read_csv('manu/background_manu.csv', header=None).sample(frac=1)
    #------------------------------------DTL intervention-1-------------------------------
    if DTL: 
        signal = signal[14000:]
...
#for tp_ in listas:
#    for vp_ in listas:
#        make_dataset(0.5,vp_,tp_,False,dire='manu_sin_cuadrinorm')
make_dataset(0.5,0.005,0.98,False,dire='manu_sin_cuadrinorm')
