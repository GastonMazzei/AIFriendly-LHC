...
class AI_model:
  def __init__(self, tp, **kwargs):
    ...
    self.X_val = X_val    # igual no lo estamos usando ...
    self.y_val = y_val    #el validation set no esta siendo usado...
    self.X_test = X_test
    self.y_test = y_test
    self.history = None
  ...
  def trainModel(self): # Should we use CAMMEL CASE for class_functions? vote NO
    ...
    #
    try:  self.soft_auc = metrics.roc_auc_score(self.y_test, self.model.predict_proba(self.X_test))
    except ValueError:
      print('PROBLEM WITH SOFT_AUC... replacing it for "None"')
      print(f'\n\nby the way... you should know the ACTIVATOR: {self.activation}\n\n')
      self.soft_auc = None
    try:
      ...
      self.f1 = None
    #
    # a new "thing" en donde guardas vectores!
    try: self.vector_roc =  [list(x) for x in roc_curve(self.y_test, self.model.predict_proba(self.X_test))]
    except Exception as ins:
      print('WARNING: vector_roc encountered problems and was overwritten with [0]')
      print(ins.args)
      self.vector_roc = [0]
    self.vector_loss = self.history.history['loss']
    self.vector_lossv = self.history.history['val_loss']
    self.vector_accuracy = self.history.history['accuracy']
    self.vector_accuracyv = self.history.history['val_accuracy']
    try:
      self.vector_hardroc = [list(x) for x in roc_curve(self.y_test, self.model.predict_classes(self.X_test))]
    except Exception as ins:
      print('WARNING: vector_hardroc encountered problems and was overwritten with [0]')
      self.vector_hardroc = [0]
    #  end
    #
    return 
