...
def loader(nm):
  q = pd.read_csv('../new-data/'+nm,low_memory=False,
         header=None)  
  if ((len(q) == 13300) and (
...
def main(**kwargs):
  ...
  return history, f'{deep}{name}T{ptrain}-V{pval}',(data['train'],batch)
...
if __name__=='__main__':
  ...
  GLOBALDIR = 'results'
  try: os.chdir(GLOBALDIR)
  except:
    try: 
      os.mkdir(GLOBALDIR)
      os.chdir(GLOBALDIR)
    except Exception as ins:
      print('something went wrong: permissions?')
      print(ins.args)
      sys.exit(1)
  ...
  try:
    ...
    print(ins.args)
    print('SYS ARGV WAS EMPTY')
    for EPO in [128]:
      for BA in [16]:
        for PTR in [50,2]:
          for PVA in [50]:
            for NEU in [4,8,16,32,64]:
              for ne in [0,'clean-','cartesian-','spherical-']:
                for dp in [0,1]:
                  print(f'\nCURRENTLY RUNNING: ',EPO,BA,PTR,PVA,NEU,ne,dp)
                  backend_wrapper(
                  lambda: simple_plotter(*pickle_bridge(
                        *main(
                               ptrain= PTR, pval= PVA,
                               ...
                               batch=BA,  verbose=0, 
                               name=ne, deep=dp,       ))))
    for EPO in [128,256,512,1024]:
      for BA in [32,64,128,512]:
        for PTR in [50,2]:
          for PVA in [50,2]:
            for NEU in [4,8,16,32,64]:
              for ne in [0,'clean-','cartesian-','spherical-']:
